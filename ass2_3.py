...
#
states = (('jscommentline', 'exclusive'),
          ('jscommentdelim', 'exclusive'),
)

t_ignore = ' \t\v\r' # whitespace 

def t_jscommentline(token):
    r'//'
    token.lexer.begin('jscommentline')

def t_jscommentline_end(token):
    r'\n'
    token.lexer.begin('INITIAL')

def t_jscommentline_error(token):
    token.lexer.skip(1)

def t_jscommentdelim(token):
    r'/\*'
    token.lexer.begin('jscommentdelim')

def t_jscommentdelim_end(token):
    r'\*/'
    token.lexer.begin('INITIAL')

def t_jscommentdelim_error(token):
    token.lexer.skip(1)
