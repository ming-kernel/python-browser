...
def csuffix(X,Y):
    ...
    if X[-1:] != Y[-1:]:
        ...
        if chart.has_key((X, Y)):
            suffix_len = chart[(X, Y)]
        else:
            suffix_len =  1 + csuffix(X[:-1], Y[:-1])
            chart[(X, Y)] = suffix_len
...
def prefixes(X):
    ...
    return [X[0:i+1] for i in xrange(len(X))]

def lsubstring(X,Y):
    # Fill in your code here (can be done in one or two lines).
    x = prefixes(X)
    y = prefixes(Y)
    return max([csuffix(a, b) for a in prefixes(X) for b in prefixes(Y)])
