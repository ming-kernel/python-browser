...
def cfgempty(grammar,symbol,visited):
    ...
    if symbol in visited:
        return None
    elif all([symbol is not rule[0] for rule in grammar]):
        return [symbol]
    else:
        new_visited = visited + [symbol]
        #consider every rewrite rule
...
def cfgempty_2(grammar, symbol, visited):
    if symbol in visited:
        return None
    elif all([rule[0] != symbol for rule in grammar]):
        return [symbol]
    else:
        new_visited = visited + [symbol]
        for rule in grammar:
            if rule[0] == symbol:
                if all([None != cfgempty(grammar, elm, new_visited) for elm in rule[1]]):
                    result = []
                    for elm in rule[1]:
                        result = result + cfgempty(grammar, elm, new_visited)
                    return result
        return None
...
print cfgempty_2(grammar1, "S", []) == None
